...
export default defineStore("diary", () => {
  ...
  const diaryData = useStorage<any[]>("diaryData", [])
  ...
  const diary = computed(() => {
    // Copy before sorting: .sort() in a computed would mutate the persisted
    // array as a side effect.
    const rawDiary = matchedDiary.value ? [...matchedDiary.value.diary] : []

    const sort: Record<string, (array: any[]) => any[]> = {
      name: (array) =>
        array.sort((a, b) => a.Name.localeCompare(b.Name)),
      score: (array) => array.sort((a, b) => b.Score - a.Score),
    }

    // Fall back for stale sortBy values persisted by older versions.
    const sortedDiary = (sort[settingsStore.settings.sortBy] ?? sort.score)(
      rawDiary
    )
