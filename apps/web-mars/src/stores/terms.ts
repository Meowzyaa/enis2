...
export default defineStore("terms", () => {
  ...
  const termsData = useStorage<any[]>("termsData", [])
  const actual = useStorage<string | null>("actualTermName", null)
  ...
  const shorterTermName = (name: string) => {
  ...
  const fetchTerms = async (force = false) => {
    ...
    try {
      ...
      // Past years may have no actual term; don't crash the whole fetch.
      const actualTerm = terms.find((term: any) => term.isActual)

      if (actualTerm) actual.value = shorterTermName(actualTerm.Name)
      settings.value.tab = settings.value.tab || actual.value || ""

      const formattedTerms = terms.map(({ Id, Name: label }: any) => ({
        Id,
